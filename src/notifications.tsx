...
export function callNotifications(
  ...
  useNotificationStore
    .getState()
    .setNotificationContent([typeNotification, content]);
  useNotificationStore.getState().setNotificationActivity(true);
}

export function NotificationComponent() {
  const notificationRef = useRef<HTMLDivElement>(null);
  const notification: boolean = useNotificationStore(
    (state) => state.notificationActivity,
  );
  ...
  useEffect(() => {
    if (notificationRef.current !== null) {
      if (notification) {
        setTypeNotification(
          useNotificationStore.getState().notificationContent
            .typeNotification as "error" | "success",
        );
        setContent(
          useNotificationStore.getState().notificationContent.content as string,
        );
  ...
  return (
    ...
    >
      <div className="flex flex-row gap-[clamp(10px,2vw,30px)] justify-between">
        <p
          className="font-semibold text-[clamp(0.75rem,1.5vw,1.5rem)]"
          style={{ color: `var(--color-notification-${typeNotification})` }}
        >
        ...
        <span
          ...
          className="material-symbols-outlined cursor-pointer scale-[0.5] md:scale-[1]"
        >
          close
        </span>
      </div>
