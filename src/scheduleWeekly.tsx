...
export function ScheduleWeeklyComponent({tasksWeekly} : {
        ...
        tasksWeekly.map((day : TasksCollectionIntf, index) => (
            <div key={index} className="flex flex-col gap-[clamp(5px,2vh,20px)] bg-plate-accent px-[clamp(5px,1vw,20px)] py-[clamp(5px,2vh,20px)] rounded-[12px] h-fit w-fit">
                ...
                <div className="flex flex-col gap-[clamp(5px,2vh,20px)] w-[30vw] max-h-[70vh] overflow-y-auto">
                    {
                        day.tasks[0] == undefined ? <p className="text-[clamp(1rem,2vw,2rem)] text-center text-content font-medium">Задач нету</p> : day.tasks.map((el : TaskIntf, index) => (
